...
class VAE(nn.Module):
    def __init__(self,
        ...
        user_num, embedvec = rating_mat.shape
        self.user_num = user_num
        self.item_num = item_num
        self.rating_mat = rating_mat
        # self.in_dims = in_dims
        p_dims = [50, 100, item_num]
        self.p_dims = p_dims
        if q_dims:
            assert q_dims[0] == in_dims, "In and Out dimensions must equal to each other"
            assert q_dims[-1] == p_dims[0], "Latent dimension for p- and q- network mismatches."
            self.q_dims = q_dims
        else:
            self.q_dims = [in_dims, 100, 50]
    ...
    def fit(self, train_loader):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for _, ur, mask_ur, embed in pbar:
                if torch.cuda.is_available():
                    ur = ur.cuda()
                ...
                embed = embed.float()
                pred, mu, logvar = self.forward(embed)
                # BCE
                loss = -torch.mean(torch.sum(F.log_softmax(pred, 1) * ur, -1))
...
class CVAE(nn.Module):
    def __init__(self,
        ...
        self.item_num = item_num
        self.rating_mat = rating_mat
        self.intent_embed = intent_embed.cuda()
        
        self.intent_dim = intent_embed.shape[1]
        # self.in_dims = in_dims
        p_dims = [50+self.intent_dim, 100, item_num]
        # p_dims = [50, 100, item_num]
        # in_dims = in_dims + self.intent_dim
        self.p_dims = p_dims
        if q_dims:
            assert q_dims[0] == in_dims, "In and Out dimensions must equal to each other"
            assert q_dims[-1] == p_dims[0], "Latent dimension for p- and q- network mismatches."
    ...
    def forward(self, input, intent_embed):
        # input = torch.cat((input, intent_embed), dim=1)
        mu, logvar = self.encode(input)
        z = self.reparameterize(mu, logvar)
        z = torch.cat((z, intent_embed), dim=1)
        z = self.decode(z)
        return z, mu, logvar
    ...
    def fit(self, train_loader):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            pbar = tqdm(train_loader)
            pbar.set_description(f'[Epoch {epoch:03d}]')
            for _, ur, mask_ur, embed, intent_embed in pbar:
                if torch.cuda.is_available():
                    ur = ur.cuda()
                    mask_ur = mask_ur.cuda()
                    embed = embed.cuda()
                    intent_embed = intent_embed.cuda()
                else:
                    ur = ur.cpu()
                    mask_ur = mask_ur.cpu()
                    embed = embed.cpu()
                    intent_embed = intent_embed.cpu()
                self.zero_grad()
                ur = ur.float()
                mask_ur = mask_ur.float()
                embed = embed.float()
                pred, mu, logvar = self.forward(embed, intent_embed)
                # BCE
                loss = -torch.mean(torch.sum(F.log_softmax(pred, 1) * ur, -1))
                # loss = criterion(pred * mask_ur, ur * mask_ur)
                KLD = -self.beta * torch.mean(torch.sum(1 + logvar - mu.pow(2) - logvar.exp(), dim=1))
        ...
        # consider there is no enough GPU memory to calculate, we must turn to other methods
        self.prediction = self.forward(x_items, self.intent_embed)[0]
        # self.prediction = self.forward(x_items)[0]
        self.prediction.clamp_(min=0, max=5)
        self.prediction = self.prediction.cpu().detach().numpy()
